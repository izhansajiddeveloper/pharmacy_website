...
// Fetch stats
// Total pharmacists
$result_pharmacists = mysqli_query($conn, "SELECT COUNT(*) AS total_pharmacists FROM users WHERE role='pharmacist'");
$pharmacists = mysqli_fetch_assoc($result_pharmacists)['total_pharmacists'];
...
// Total stock items
$result_stock = mysqli_query($conn, "SELECT SUM(quantity) AS total_stock FROM stock_batches");
$stock = mysqli_fetch_assoc($result_stock)['total_stock'] ?: 0;

// Total sales
$result_sales = mysqli_query($conn, "SELECT COUNT(*) AS total_sales FROM sales");
$sales = mysqli_fetch_assoc($result_sales)['total_sales'];

// Today's sales revenue
$today_sales = mysqli_query($conn, "SELECT SUM(total_amount) AS revenue FROM sales WHERE DATE(sale_date) = CURDATE()");
$today_revenue = mysqli_fetch_assoc($today_sales)['revenue'] ?: 0;

// This week's revenue
$week_sales = mysqli_query($conn, "SELECT SUM(total_amount) AS revenue FROM sales WHERE YEARWEEK(sale_date) = YEARWEEK(CURDATE())");
$week_revenue = mysqli_fetch_assoc($week_sales)['revenue'] ?: 0;

// This month's revenue
$month_sales = mysqli_query($conn, "SELECT SUM(total_amount) AS revenue FROM sales WHERE MONTH(sale_date) = MONTH(CURDATE()) AND YEAR(sale_date) = YEAR(CURDATE())");
...
// Medicine categories for pie chart
$categories_query = mysqli_query(
    $conn,
    "SELECT mc.name AS category_name, COUNT(*) AS count
     FROM medicines m
...
// Sales by hour for bar chart
$hourly_sales = [];
$hourly_query = mysqli_query(
    $conn,
...
for ($i = 9; $i <= 20; $i++) {
    $hourly_sales[$i] = ['hour' => $i, 'total_sales' => 0, 'revenue' => 0];
}

while ($row = mysqli_fetch_assoc($hourly_query)) {
    $hour = (int)$row['hour'];
    if ($hour >= 9 && $hour <= 20) {
        $hourly_sales[$hour] = $row;
    }
}
...
$recent_sales = mysqli_query(
    $conn,
    "SELECT s.id, s.total_amount, s.sale_date, u.name as pharmacist_name
     FROM sales s
...
$pharmacist_performance = mysqli_query(
    $conn,
    "SELECT u.name, COUNT(s.id) as total_sales, SUM(s.total_amount) as total_revenue
     FROM sales s
     JOIN users u ON s.pharmacist_id = u.id
     WHERE s.sale_date >= DATE_SUB(NOW(), INTERVAL 30 DAY)
     GROUP BY s.pharmacist_id
     ORDER BY total_revenue DESC
     LIMIT 5"
);

// System status
$active_pharmacists = mysqli_query(
    $conn,
...
$active_count = mysqli_fetch_assoc($active_pharmacists)['active_count'] ?: 0;
        ...
        <aside id="sidebar" class="bg-gradient-to-b from-gray-900 to-gray-800 text-white fixed lg:sticky top-12 h-full lg:h-[calc(100vh-4rem)] w-64 lg:w-72 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 z-40 overflow-y-auto custom-scrollbar shadow-2xl">
            <div class="p-6">
                ...
                <nav class="space-y-2">
                    ...
                    <a href="users.php" class="flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-gray-700/50 transition-all duration-200">
                        <div class="w-8 h-8 rounded-lg bg-gray-700/50 flex items-center justify-center">
                            <i class="fas fa-user-md text-gray-300 text-lg"></i>
                        </div>
                        <span class="font-medium text-gray-200">Pharmacists</span>
                        <span class="ml-auto bg-yellow-500/20 text-yellow-300 text-xs px-2 py-1 rounded-full font-medium"><?php echo $pharmacists; ?></span>
                    </a>
                    ...
                    <a href="inventory.php" class="flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-gray-700/50 transition-all duration-200">
                        <div class="w-8 h-8 rounded-lg bg-gray-700/50 flex items-center justify-center">
                            <i class="fas fa-boxes text-gray-300 text-lg"></i>
                        </div>
                        <span class="font-medium text-gray-200">Inventory</span>
                    ...
                    <a href="sales.php" class="flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-gray-700/50 transition-all duration-200">
                        ...
                        <span class="ml-auto bg-gray-600/50 text-gray-300 text-xs px-2 py-1 rounded-full font-medium"><?php echo $sales; ?></span>
                    </a>

                    <a href="reports.php" class="flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-gray-700/50 transition-all duration-200">
                        <div class="w-8 h-8 rounded-lg bg-gray-700/50 flex items-center justify-center">
                            <i class="fas fa-chart-line text-gray-300 text-lg"></i>
                        </div>
                        <span class="font-medium text-gray-200">Reports</span>
                    </a>

                    <a href="settings.php" class="flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-gray-700/50 transition-all duration-200">
                        <div class="w-8 h-8 rounded-lg bg-gray-700/50 flex items-center justify-center">
                            <i class="fas fa-cog text-gray-300 text-lg"></i>
                        </div>
                        <span class="font-medium text-gray-200">Settings</span>
                    </a>
                    ...
                    <a href="expenses.php" class="flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-gray-700/50 transition-all duration-200">
                        <div class="w-8 h-8 rounded-lg bg-gray-700/50 flex items-center justify-center">
                            <i class="fas fa-money-bill-wave text-gray-300 text-lg"></i>
        ...
        <main class="flex-1 overflow-hidden">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center justify-between">
                    <div>
                        <h1 class="text-3xl lg:text-4xl font-bold text-gray-800 mb-2">
                            <span class="bg-gradient-to-r from-yellow-500 to-yellow-600 bg-clip-text text-transparent">Dashboard</span> Overview
                        </h1>
                        <p class="text-gray-600 flex items-center space-x-2">
                            ...
                            <span><?php echo $active_count; ?> active users</span>
                        </p>
                    </div>
            ...
            <div class="dashboard-container">
                    ...
                    <div class="chart-container rounded-2xl p-6 animate-fade-in-up" style="animation-delay: 0.5s">
                        <div class="flex flex-col lg:flex-row lg:items-center justify-between mb-6">
                            <div>
                                ...
                                <p class="text-gray-600">Revenue and sales count over last 6 months</p>
                            </div>
                ...
                <div class="grid-col-4">
                    <div class="chart-container rounded-2xl p-6 animate-fade-in-up" style="animation-delay: 0.6s">
                        <div class="flex items-center justify-between mb-6">
                            <h3 class="text-xl font-bold text-gray-800">Medicine Categories</h3>
                            <span class="text-sm text-yellow-600 font-medium bg-yellow-50 px-3 py-1 rounded-full">Top 5</span>
                        </div>
                ...
                <div class="grid-col-6">
                    <div class="chart-container rounded-2xl p-6 animate-fade-in-up" style="animation-delay: 0.7s">
                        <div class="flex items-center justify-between mb-6">
                            <div>
                                <h3 class="text-xl font-bold text-gray-800">Hourly Sales Today</h3>
                                <p class="text-gray-600">Sales distribution by hour (9 AM - 8 PM)</p>
                            </div>
                    ...
                    <div class="chart-container rounded-2xl p-6 animate-fade-in-up" style="animation-delay: 0.8s">
                        <div class="flex items-center justify-between mb-6">
                            <div>
                                ...
                                <p class="text-gray-600">Last 7 days performance</p>
                            </div>
                ...
                <div class="grid-col-4">
                    <div class="chart-container rounded-2xl p-6 animate-fade-in-up" style="animation-delay: 0.9s">
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <span class="text-sm text-purple-600 bg-purple-50 px-3 py-1 rounded-full">This Month</span>
                        </div>
                        <div class="space-y-4">
                ...
                <div class="grid-col-4">
                    <div class="chart-container rounded-2xl p-6 animate-fade-in-up" style="animation-delay: 1s">
                        <div class="flex items-center justify-between mb-6">
                            <h3 class="text-xl font-bold text-gray-800">Top Pharmacists</h3>
                            <span class="text-sm text-blue-600 bg-blue-50 px-3 py-1 rounded-full">Last 30 Days</span>
                        </div>
                        <div class="space-y-4">
                            <?php if (mysqli_num_rows($pharmacist_performance) > 0): ?>
                                <?php while ($pharmacist = mysqli_fetch_assoc($pharmacist_performance)): ?>
                                    <div class="p-3 bg-gradient-to-r from-blue-50 to-white rounded-lg border border-blue-100">
                                        <div class="flex items-center justify-between mb-2">
                                            <div class="flex items-center space-x-2">
                                                ...
                                                <span class="font-semibold text-gray-800 text-sm"><?php echo htmlspecialchars($pharmacist['name']); ?></span>
                                            </div>
                                            <span class="font-bold text-blue-600">Rs <?php echo number_format($pharmacist['total_revenue']); ?></span>
                                        </div>
                                        <div class="flex justify-between text-xs text-gray-500">
                                            <span><?php echo $pharmacist['total_sales']; ?> sales</span>
                                            <span>Avg: Rs <?php echo $pharmacist['total_sales'] > 0 ? number_format($pharmacist['total_revenue'] / $pharmacist['total_sales']) : 0; ?></span>
                                        </div>
                                    </div>
                                <?php endwhile; ?>
                            <?php else: ?>
                ...
                <div class="grid-col-12">
                    <div class="chart-container rounded-2xl p-6 animate-fade-in-up" style="animation-delay: 1.2s">
                        <div class="flex items-center justify-between mb-6">
                            <h3 class="text-xl font-bold text-gray-800">Recent Sales Activity</h3>
                                        ...
                                        <div class="flex items-center justify-between mb-3">
                                            <div class="flex items-center space-x-2">
                                                ...
                                                <div>
                                                    ...
                                                    <div class="text-sm text-gray-500"><?php echo htmlspecialchars($sale['pharmacist_name']); ?></div>
                                                </div>
                                            </div>
                                            <div class="text-right">
    ...
    <div id="quickStatsPanel" class="fixed bottom-24 right-6 bg-white rounded-2xl shadow-2xl p-6 z-50 hidden w-80">
        ...
        <div class="space-y-4">
            <div class="flex items-center justify-between">
                ...
                <span class="font-bold text-gray-800"><?php echo $active_count; ?></span>
            </div>
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-2">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const pieChart = new Chart(pieCtx, {
                type: 'pie',
                data: {
                    labels: <?php echo json_encode(array_column($categories_data, 'category_name')); ?>,
                    datasets: [{
            ...
            const hourlyChart = new Chart(hourlyCtx, {
                ...
                data: {
                    ...
                    datasets: [{
                        label: 'Sales Count',
                        data: <?php echo json_encode(array_column($hourly_sales, 'total_sales')); ?>,
                        backgroundColor: Array(12).fill().map((_, i) =>
                            i % 2 === 0 ? yellowColors[0] : grayColors[0]
                        ),
                        borderColor: Array(12).fill().map((_, i) =>
                            i % 2 === 0 ? '#f59e0b' : '#6b7280'
                        ),
                        borderWidth: 2,
                        borderRadius: 6,
